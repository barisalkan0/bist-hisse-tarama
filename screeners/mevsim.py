...
Uzun aylık geçmişten (adj_close) her takvim ayının davranışını çıkarır:
  ...
  - İsabet           : o ayın pozitif olduğu yıl ORANI (%) — Yıl ile birlikte okunur
...
def stock_seasonality(df):
    ...
    for m in range(1, 13):
        ...
        rows.append({
            ...
            # İsabet = pozitif yıl ORANI (%). Sayisal -> dogru siralanir (10/15=%67 > 8/14=%57).
            "İsabet": round(pos / yrs * 100, 1) if yrs else 0.0,
            "Yıl": yrs,
        })
...
def month_scan(all_monthly, month, min_years=7, strong=True):
    ...
    for sym, df in all_monthly.items():
        ...
        rows.append({
            ...
            "Medyan %": round(med, 2),
            "Mevsimsel Fark %": round(avg - overall, 2),
            # İsabet = pozitif yıl ORANI (%). Sayisal -> sutuna tiklayinca dogru siralanir.
            "İsabet": round(pos / yrs * 100, 1) if yrs else 0.0,
            "Yıl": yrs,
        })
