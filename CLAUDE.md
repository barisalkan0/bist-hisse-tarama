...
# Claude-First Memory and Security Workflow

Primary target: Claude Code. Codex may use the same workflow as a secondary fallback.

## Mandatory Memory Workflow
1. Start every fresh/resumed chat by reading `docs/AI_MEMORY.md` if it exists.
2. Use `codebase-memory-mcp` before broad file reads or recursive search.
3. MCP project id: `C-Users-asus-OneDrive-Masa-st-ahin-hisse-takip`.
4. Current MCP coverage is mostly file-tree level for this repo; use it for orientation, then read only targeted files.
5. After significant changes, run available checks, run `detect_changes` when useful, update `docs/AI_MEMORY.md`, and update `docs/SECURITY_NOTES.md` if security, database, subscription, payment, auth, or financial-disclaimer behavior changed.
6. Keep the existing Turkish project notes above as product/domain truth unless current source code contradicts them.

## High-Risk Security Rules
- Treat this repo as `high-risk`: it is a financial-market decision-support product and the user plans subscription, database, and public launch soon.
- Never present output as investment advice. Keep language as candidate/watch/risk/decision support.
- Never say "secure", "production-ready", or "safe to launch" unless the relevant checks were actually performed. If not, say: "Not verified yet."
- Do not store or repeat secrets, tokens, payment data, customer data, private user notes, API keys, Redis tokens, DB URLs, webhook secrets, or raw PII in memory notes, ADRs, prompts, logs, or examples.
- Do not read `.streamlit/secrets.toml`, local env files, or secret-bearing config unless the user explicitly asks and the task truly requires it.
- Before subscription/payment launch, require server-side subscription gating, payment provider webhook signature verification, webhook idempotency, database row-level access rules, backups, audit logs, and legal/financial disclaimers.
